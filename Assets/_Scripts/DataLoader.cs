...
public class DataLoader : MonoBehaviour {
	...
	[field: SerializeField] public MobScriptable[] MobScriptables { get; private set; }
	#endregion
	...
	public void UpdateDatas() {
		...
		guids = AssetDatabase.FindAssets("t:MobScriptable", folders);
		MobScriptables = new MobScriptable[guids.Length];
		for (int i = 0; i < guids.Length; i++)
			MobScriptables[i] = AssetDatabase.LoadAssetAtPath<MobScriptable>(AssetDatabase.GUIDToAssetPath(guids[i]));
	}
#endif
	...
	public CharacterModel LoadMobModel(int id) {
		if (id == GameUtilsAndConsts.EMPTY_INT)
			return CharacterModel.Empty;

		MobScriptable mob = Array.Find(MobScriptables, x => x.Id == id);
		return new CharacterModel(mob.Id.ToString(), mob.StatModel);
	}
}
